...
bool DBusAdapter::init() {
    ...
    if (!connection_) {
        LOG_E << "Failed to connect to DBus: " << error->message;
        g_error_free(error);
        return false;
    }
    
    connected_ = true;
    LOG_I << "Connected to DBus system bus";
...
uint32_t DBusAdapter::subscribeSignal(const std::string& interface,
    ...
    LOG_I << "Subscribed to DBus signal: " << interface << "." << member;
...
void DBusAdapter::unsubscribeSignal(uint32_t subscriptionId) {
    ...
    if (it != subscriptions_.end()) {
        ...
        LOG_D << "Unsubscribed from signal: " << subscriptionId;
    }
}
...
bool DBusAdapter::sendSignal(const std::string& interface,
    ...
    LOG_D << "Sent DBus signal: " << interface << "." << member;
    return true;
}
...
void DBusAdapter::onDBusSignal(_GDBusConnection* connection,
    ...
    LOG_D << "Received DBus signal: " << signal_name;
}
...
void DBusAdapter::cleanup() {
    ...
    LOG_I << "DBus connection cleaned up";
}
